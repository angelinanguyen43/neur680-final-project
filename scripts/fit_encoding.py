...
"""
fit_encoding.py
Voxel-wise ridge regression for ONE DNN layer, memory-safe on a laptop.
...
# ────────── load DNN features (on-disk, not in RAM) ──────────
...
for subj in subjects:
    ...
    for r in sorted(runs, key=lambda r: r.path):
        ...
        if idx.size == 0:
            continue
        X_rows.append(idx)
        Y_parts.append(np.load(cleaned_npy(r.path, subj)))
    ...
    # ── slice *once*, then copy to float32 ──
    ...
    for start in range(0, Y_all.shape[1], VOX_BATCH):
        ...
        if (start // VOX_BATCH) % 20 == 0:
            LOGGER.info("    voxels %d-%d / %d",
                        start, stop, Y_all.shape[1])
